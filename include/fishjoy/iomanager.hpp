...
class IOManager : public Scheduler, public TimerManager {
 ...
 public:
  ...
  /**
   * @brief 添加事件
   * @details fd描述符发生了event事件时执行callback函数
   * @param fd socket 句柄
   * @param event 事件类型
   * @param callback 时间回调函数，如果为空，则默认把当前协程作为回调执行体
   ...
   * @param fd socket 句柄
   * @param event 事件类型
   * @attention 如果该事件注册过回调，那就触发一次回调事件
   * @return 是否取消成功
   */
  ...
  /**
   * @brief 取消所有事件
   * @details 所有被注册的回调事件在 cancel 之间都会被执行一次
   * @param fd socket 句柄
   * @return 是否删除成功
   */
  bool cancelAll(int fa);

  /**
   * @return 返回当前的 IOManager
   */
