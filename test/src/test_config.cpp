...
void test_class() {
  ...
  if(!g_person->getListener(id)) {
    id = g_person->addListener([](const Person &old_value, const Person &new_value){
      FISHJOY_LOG_INFO(test_logger) << "g_person value change, old value:" << old_value.toString()
                               << ", new value:" << new_value.toString();
    });
  }

  FISHJOY_LOG_INFO(test_logger) << g_person->getValue().toString();

  for (const auto &i : g_person_map->getValue()) {
    FISHJOY_LOG_INFO(test_logger) << i.first << ":" << i.second.toString();
  }

  for(const auto &i : g_person_vec_map->getValue()) {
    FISHJOY_LOG_INFO(test_logger) << i.first;
    for(const auto &j : i.second) {
      FISHJOY_LOG_INFO(test_logger) << j.toString();
    }
  }
}
...
void test_config() {
  FISHJOY_LOG_INFO(test_logger) << "g_int value: " << g_int->getValue();
  FISHJOY_LOG_INFO(test_logger) << "g_float value: " << g_float->getValue();
  FISHJOY_LOG_INFO(test_logger) << "g_string value: " << g_string->getValue();
  FISHJOY_LOG_INFO(test_logger) << "g_int_vec value: " << formatArray<std::vector<int>>(g_int_vec->getValue());
  FISHJOY_LOG_INFO(test_logger) << "g_int_list value: " << formatArray<std::list<int>>(g_int_list->getValue());
  FISHJOY_LOG_INFO(test_logger) << "g_int_set value: " << formatArray<std::set<int>>(g_int_set->getValue());
  FISHJOY_LOG_INFO(test_logger) << "g_int_unordered_set value: " << formatArray<std::unordered_set<int>>(g_int_unordered_set->getValue());
  FISHJOY_LOG_INFO(test_logger) << "g_int_map value: " << formatMap<std::map<std::string, int>>(g_map_string2int->getValue());
  FISHJOY_LOG_INFO(test_logger) << "g_int_unordered_map value: " << formatMap<std::unordered_map<std::string, int>>(g_unordered_map_string2int->getValue());
...
int main(int argc, char *argv[]) {
  ...
  g_int->addListener([](const int &old_value, const int &new_value) {
    FISHJOY_LOG_INFO(test_logger) << "g_int value changed, old_value: " << old_value << ", new_value: " << new_value;
  });

  FISHJOY_LOG_INFO(test_logger) << "before============================";
  ...
  FISHJOY_LOG_INFO(test_logger) << "after============================";
    
  test_config();

  // 遍历所有配置
  fishjoy::Config::Visit([](fishjoy::ConfigVarBase::ptr var){
    FISHJOY_LOG_INFO(test_logger) << "name=" << var->getName()
