...
void run_in_fiber() {
  ...
  FISHJOY_LOG_INFO(g_logger) << "before run_in_fiber yield";
  fishjoy::Fiber::GetThis()->yield();
  FISHJOY_LOG_INFO(g_logger) << "after run_in_fiber yield";
...
void test_fiber() {
  FISHJOY_LOG_INFO(g_logger) << "test_fiber begin";

  // 初始化线程主协程
  fishjoy::Fiber::GetThis();

  fishjoy::Fiber::ptr fiber(new fishjoy::Fiber(run_in_fiber, 0, false));
  FISHJOY_LOG_INFO(g_logger) << "use_count:" << fiber.use_count(); // 1

  FISHJOY_LOG_INFO(g_logger) << "before test_fiber resume";
  fiber->resume();
  FISHJOY_LOG_INFO(g_logger) << "after test_fiber resume";
   ...
   */
  FISHJOY_LOG_INFO(g_logger) << "use_count:" << fiber.use_count(); // 3

  FISHJOY_LOG_INFO(g_logger) << "fiber status: " << fiber->getState(); // READY

  FISHJOY_LOG_INFO(g_logger) << "before test_fiber resume again";
  fiber->resume();
  FISHJOY_LOG_INFO(g_logger) << "after test_fiber resume again";

  FISHJOY_LOG_INFO(g_logger) << "use_count:" << fiber.use_count(); // 1
  FISHJOY_LOG_INFO(g_logger) << "fiber status: " << fiber->getState(); // TERM

  fiber->reset(run_in_fiber2); // 上一个协程结束之后，复用其栈空间再创建一个新协程
  fiber->resume();

  FISHJOY_LOG_INFO(g_logger) << "use_count:" << fiber.use_count(); // 1
  FISHJOY_LOG_INFO(g_logger) << "test_fiber end";
}
