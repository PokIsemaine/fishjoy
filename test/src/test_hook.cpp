 ...
 */
void test_sleep() {
  FISHJOY_LOG_INFO(test_logger) << "test_sleep begin";
  ...
  iom.schedule([] {
    sleep(2);
    FISHJOY_LOG_INFO(test_logger) << "sleep 2";
  });

  iom.schedule([] {
    sleep(3);
    FISHJOY_LOG_INFO(test_logger) << "sleep 3";
  });

  FISHJOY_LOG_INFO(test_logger) << "test_sleep end";
}
...
void test_sock() {
  ...
  FISHJOY_LOG_INFO(test_logger) << "begin connect";
  int rt = connect(sock, (const sockaddr*)&addr, sizeof(addr));
  FISHJOY_LOG_INFO(test_logger) << "connect rt=" << rt << " errno=" << errno;
  ...
  FISHJOY_LOG_INFO(test_logger) << "send rt=" << rt << " errno=" << errno;
  ...
  FISHJOY_LOG_INFO(test_logger) << "recv rt=" << rt << " errno=" << errno;
  ...
  buff.resize(rt);
  FISHJOY_LOG_INFO(test_logger) << buff;
}
