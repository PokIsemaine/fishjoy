...
int main() {
  ...
  /** 
     ...
     * 消耗掉，然后再返回main函数往下执行
   */
  fishjoy::Scheduler sc; 

  // 额外创建新的线程进行调度，那只要添加了调度任务，调度器马上就可以调度该任务
  // fishjoy::Scheduler sc(3, false);
  ...
  fishjoy::Fiber::ptr fiber(new fishjoy::Fiber(&test_fiber3));
  sc.schedule(fiber);
  ...
  /**
     * 只要调度器未停止，就可以添加调度任务
     * 包括在子协程中也可以通过fishjoy::Scheduler::GetThis()->scheduler()的方式继续添加调度任务
   */
