...
namespace fishjoy {
  ...
  Timer::Timer(uint64_t ms, std::function<void()> cb,
      ...
      :m_recurring(recurring)
        ...
        ,m_cb(cb)
        ,m_manager(manager) {
    m_next = fishjoy::GetElapsedMS() + m_ms;
  }
  ...
  TimerManager::TimerManager() {
    m_previousTime = fishjoy::GetElapsedMS();
  }
  ...
  uint64_t TimerManager::getNextTimer() {
    ...
    uint64_t now_ms = fishjoy::GetElapsedMS();
  ...
  void TimerManager::listExpiredCb(std::vector<std::function<void()> >& cbs) {
    ...
    bool rollover = false;
    if(FISHJOY_UNLIKELY(detectClockRollover(now_ms))) {
