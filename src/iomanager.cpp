...
#undef XX
  ...
  int IOManager::addEvent(int fd, Event event, std::function<void()> callback) {
    //  找到 fd 对应的 FdContext 如果不存在，那就分配一个
    ...
    // 同一个 fd 不允许重复添加相同的事件
    if (FISHJOY_UNLIKELY(fd_ctx->events & event)) {
    ...
    if (rt) {
      FISHJOY_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
  ...
  bool IOManager::cancelAll(int fd) {
    ...
    FISHJOY_ASSERT(fd_ctx->events == 0);
    return true;
  }
 ...
 * 如果没有调度线程处理于idle状态，那也就没必要发通知了
  ...
  void IOManager::tickle() {
    FISHJOY_LOG_DEBUG(g_logger) << "tickle";
    if(!hasIdleThreads()) {
      return;
    }
  ...
  void IOManager::idle() {
    ...
    while (true) {
      ...
      if( FISHJOY_UNLIKELY(stopping(next_timeout))) {
        FISHJOY_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
        break;
      }
