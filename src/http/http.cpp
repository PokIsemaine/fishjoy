...
#define PARSE_PARAM(str, m, flag, trim)                                                                    \
    ...
    do {                                                                                                   \
        ...
        if (0) {                                                                                           \
            std::cout << "<key>:" << str.substr(last, key - last)                                          \
                      << " <decoded>:" << fishjoy::StringUtil::UrlDecode(str.substr(last, key - last))       \
                      << " <value>:" << str.substr(key + 1, pos - key - 1)                                 \
                      << " <decoded>:" << fishjoy::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1)) \
                      << std::endl;                                                                        \
        }                                                                                                  \
                                                                                                           \
        m.insert(std::make_pair(fishjoy::StringUtil::UrlDecode(trim(str.substr(last, key - last))),          \
                                fishjoy::StringUtil::UrlDecode(str.substr(key + 1, pos - key - 1))));        \
        if (pos == std::string::npos) {                                                                    \
            break;                                                                                         \
        }                                                                                                  \
        ++pos;                                                                                             \
    ...
    void HttpRequest::initCookies() {
      ...
      PARSE_PARAM(cookie, m_cookies, ';', fishjoy::StringUtil::Trim);
      m_parserParamFlag |= 0x4;
    }
    ...
    void HttpResponse::setCookie(const std::string &key, const std::string &val,
      ...
      ss << key << "=" << val;
      if (expired > 0) {
        ss << ";expires=" << fishjoy::Time2Str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
      }
      if (!domain.empty()) {
        ss << ";domain=" << domain;
      }
