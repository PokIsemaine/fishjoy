...
namespace fishjoy {
  ...
  bool Address::Lookup(std::vector<Address::ptr> &result, const std::string &host, int family, int type, int protocol) {
    ...
    if (error) {
      FISHJOY_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", " << family << ", " << type
    ...
    while (next) {
      ...
      FISHJOY_LOG_DEBUG(g_logger) << "family:" << next->ai_family << ", sock type:" << next->ai_socktype;
      next = next->ai_next;
    }
  ...
  bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>> &result, int family) {
    ...
    if (getifaddrs(&results) != 0) {
      FISHJOY_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                                   " err="
    ...
    try {
      ...
      FISHJOY_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
      freeifaddrs(results);
      return false;
    }
    freeifaddrs(results);
  ...
  IPAddress::ptr IPAddress::Create(const char *address, uint16_t port) {
    ...
    if (error) {
      FISHJOY_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address << ", " << port << ") error=" << error
  ...
  IPv4Address::ptr IPv4Address::Create(const char *address, uint16_t port) {
    ...
    if (result <= 0) {
      FISHJOY_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", " << port << ") rt=" << result
                                << " errno=" << errno << " errstr=" << strerror(errno);
      return nullptr;
    }
    return rt;
  ...
  IPv6Address::ptr IPv6Address::Create(const char *address, uint16_t port) {
    ...
    if (result <= 0) {
      FISHJOY_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", " << port << ") rt=" << result
                                << " errno=" << errno << " errstr=" << strerror(errno);
      return nullptr;
    }
    return rt;
