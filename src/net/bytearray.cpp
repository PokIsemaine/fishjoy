...
namespace fishjoy {
  ...
  bool ByteArray::isLittleEndian() const {
    return m_endian == FISHJOY_LITTLE_ENDIAN;
  }

  void ByteArray::setIsLittleEndian(bool val) {
    if(val) {
      m_endian = FISHJOY_LITTLE_ENDIAN;
    } else {
      m_endian = FISHJOY_BIG_ENDIAN;
    }
  }
  ...
  void ByteArray::writeFint16 (int16_t value) {
    if(m_endian != FISHJOY_BYTE_ORDER) {
...
#undef XX
  ...
  bool ByteArray::writeToFile(const std::string& name) const {
    ...
    if(!ofs) {
      FISHJOY_LOG_ERROR(g_logger) << "writeToFile name=" << name
  ...
  bool ByteArray::readFromFile(const std::string& name) {
    ...
    if(!ifs) {
      FISHJOY_LOG_ERROR(g_logger) << "readFromFile name=" << name
                                << " error, errno=" << errno << " errstr=" << strerror(errno);
      return false;
    }
